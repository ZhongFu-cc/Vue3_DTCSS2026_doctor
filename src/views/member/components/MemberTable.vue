<template>
  <section class="member-table-section">
    <el-table :data="props.memberData" @selection-change="$emit('mutli-delete-members', $event)" class="member-table">
      ...
      <el-table-column v-if="isIndexView" fixed prop="chineseName" label="姓名" width="100">
      </el-table-column>
...
console.log('props.memberData', props.memberData);

const isDevice = computed(() => useAppStore().device === 'mobile' ? true : false);
const isIndexView = computed(() => props.viewPage === 'index');
</script>
